/** \file
 A program to read in the NYT's hadline feed and producd a simple
...
/** \mainpage
 * The front page of the Grey Lady's web site is as gaudy as can be,
 * including several headlines and sections trying to get your attention, 
 * various formatting schemes, and even photographs -- in <em>color</em>.
 *
 * This program reads in the NYT Headlines RSS feed, and writes a simple 
 * list in plain HTML. You can then click through to the headline that 
 * modestly piques your attention.
 *
 ...
 * Notice that cURL has a program, \c curl-config, that behaves like 
 * \c pkg-config, but is cURL-specific.
 *
 * \code
 * CFLAGS=-g -Wall -O3 'curl-config --cflags' -I/urs/include/libxml2
 ...
 * \endcode
 *
 * Having saved your makefile, use <tt>maken nyt_feed</tt> to compile.
 *
 * Of course, you have to have the development packages for libcurl and 
 * libxml2 installed for this to work.
 */
 
 // These have in-line Doxygen documentation. The < points to the prior 
 // text being documented.
 ...
 void print_to_html(xmlXPathObjectPtr urls, xmlXPathObjectPtr titles) 
 {
     ...
     for (int i = 0; i < titles->nodesetval->nodeNr; i++) {
         fprintf(f, "<a href=\"%s\">%s</a><br>\n", xmlNodeGetContent(urls->nodesetval->nodeTab[i]),xmlNodeGetContent(urls->nodesetval->nodeTab[i]));
     fclose(f);
     }
 }
...
int parse(char const *infile)
{
    ...
    Stopif(!titles || !urls, return -3, "either the Xpath '//item/title' or '//item/link' filed.");

    print_to_html(urls, titles);

    xmlXPathFreeObject(titles);
    xmlXPathFreeObject(urls);
    xmlXPathFreContext(context);
    xmlFreeDoc(doc);
    return 0;
}
